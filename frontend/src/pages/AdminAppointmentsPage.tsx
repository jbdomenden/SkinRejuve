    ...
    <AdminShell>
      <div className='flex items-center justify-between'>
        ...
        <button
          ...
          onClick={() =>
            exportCsv('appointments-report.csv', [
              ['id', 'patient', 'service', 'date', 'status'],
              ...rows.map((row) => [row.id, row.patient, row.service, row.date, row.status]),
            ])
          }
        >
          EXPORT CSV
          ...
          <tbody>
            {rows.map((row) => (
              <tr className='border-t border-[#8a5a2f]/20' key={row.id}>
                ...
                <td>{row.status}</td>
                <td>
                  <button className='portal-btn-small' onClick={() => setSelectedId(row.id)}>
                    Review
                  </button>
                </td>
              </tr>
      ...
      {selectedId ? (
        <AdminModal
          ...
          actions={
            resolution ? (
              <button className='portal-btn-small' onClick={() => setSelectedId(null)}>
                Done
              </button>
            ) : (
              <>
                <button className='border border-[#8a5a2f]/45 px-5 py-2' onClick={() => setResolution('rejected')}>
                  Reject
                </button>
                <button className='portal-btn-small' onClick={() => setResolution('approved')}>
                  Approve
                </button>
              </>
            )
        ...
        >
          {!resolution ? (
            <div className='space-y-2'>
              <p>
                <strong>Patient:</strong> Ana Dela Cruz
              </p>
              <p>
                <strong>Requested:</strong> Acne Laser, March 17 • 09:00 AM
              </p>
              <p>
                <strong>Note:</strong> First-time consultation with recent topical treatment.
              </p>
            </div>
          ) : resolution === 'approved' ? (
